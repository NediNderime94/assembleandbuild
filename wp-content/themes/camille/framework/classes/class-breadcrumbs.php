...
class Camille_Breadcrumbs {
	...
	function __construct( $args = array() ) {
		...
		$defaults = array(
			...
			'labels'            => array(),
			'date_labels'       => array(),
			'action'            => 'camille/action/breadcrumbs/render_html',
			'css_namespace'     => array(),
			'path_type'         => 'default', // minified , full, default
			'post_taxonomy'     => apply_filters(
				'camille/filter/breadcrumbs/trail_taxonomies',
				array(
					'post'      => 'category',
					'product'	=> 'product_cat'
				)
		...
		$this->args = apply_filters( 'camille/filter/breadcrumbs/args', wp_parse_args( $args, $defaults ) );
	...
	public function get_trail() {
		...
		$wrapper_classes = apply_filters( 'camille/filter/breadcrumbs/wrapper_classes', $wrapper_classes );
		...
		do_action('camille/action/breadcrumbs/before_render_html', $this, $this->args);
	...
	public function get_items() {
		...
		$breadcrumb = apply_filters( 'camille/filter/breadcrumbs/trail', $breadcrumb, $this->args );
	...
	public function default_labels() {
		...
		$labels = array(
			'browse'              => esc_html_x( 'Browse:', 'front-view', 'camille' ),
			'home'                => $this->home_title(),
			'error_404'           => esc_html_x( '404 Not Found', 'front-view', 'camille' ),
			'archives'            => esc_html_x( 'Archives', 'front-view', 'camille' ),
			'search'              => esc_html_x( 'Search results for &#8220;%s&#8221;', 'front-view', 'camille' ),
			'paged'               => esc_html_x( 'Page %s', 'front-view', 'camille' ),
			'archive_minute'      => esc_html_x( 'Minute %s', 'front-view', 'camille' ),
			'archive_week'        => esc_html_x( 'Week %s', 'front-view', 'camille' ),
	...
	public function home_title() {
		...
		$title            = esc_html_x( 'Home', 'front-view', 'camille' );
		$use_custom_title = apply_filters( 'camille/filter/breadcrumbs/custom_home_title', false );
	...
	public function build_trail() {
		...
		$custom_trail = apply_filters( 'camille/filter/breadcrumbs/custom_trail', false, $this->args );
		 ...
		 */
		$this->items = apply_filters( 'camille/filter/breadcrumbs/items', $this->items, $this->args, $this );
	...
	public function add_post_terms( $post_id ) {
		...
		if ( ! empty( $this->args['post_taxonomy'][ $post_type ] ) ) {
			add_filter('get_object_terms', 'camille_exclude_demo_term_in_category');
			$post_terms = wp_get_post_terms( $post_id, $this->args['post_taxonomy'][ $post_type ] );
			remove_filter('get_object_terms', 'camille_exclude_demo_term_in_category');
	...
	public function prepare_label( $label, $fallback = false ) {
		...
		if ( false !== $fallback ) {
			$fallback = sprintf(
				apply_filters( 'camille/filter/breadcrumbs/icon_fallback', '<span class="hidden">%s</span>', $fallback ),
				esc_attr( $fallback )
			);
		}
		...
		// Return default icon
		return sprintf(
			apply_filters( 'camille/filter/breadcrumbs/default_icon_format', '<span class="%1$s">%2$s</span>', $label, $fallback ),
	...
	public function get_breadcrumb() {
		return apply_filters( 'camille/filter/breadcrumbs/get_breadcrumb', $this->crumbs, $this );
	}
}