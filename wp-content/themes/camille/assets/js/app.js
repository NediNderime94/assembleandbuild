...
(function($) {
    ...
    LA.component.AnimationBlock = function(el){
        ...
        var render_transition_delay = function(delay){
            var _attr_rendered = '';
            _attr_rendered += '-webkit-animation-delay:' + delay + 's;';
            _attr_rendered += '-moz-animation-delay:' + delay + 's;';
            _attr_rendered += 'animation-delay:' + delay + 's;';
            return _attr_rendered;
        }

        var init = function(){

            LA.utils.LazyLoad($(el), {
                load : function (sc) {
                    var $this = $(sc);
                    if(false !== !!$this.data('animate')){
                        var $childs = $this.find('>div:not(.la-animation-block):not([data-la_component])'),
                            animationName = $this.data('animate'),
                            animationDuration = $this.data('animation-duration')+'s',
                            animationIteration = parseInt($this.data('animation-iteration')),
                            animationDelay = parseFloat($this.data('animation-delay'));
                        ...
                        if( $this.is('.wpb_wrapper') ) {
                            var $parent = $(sc).closest('.wpb_column'),
                                parent_stype = $parent.attr('style') || '';
                            parent_stype = parent_stype.replace(/ /g,'');
                            if(parent_stype == 'opacity:0;'){
                                if( parent_stype.indexOf(container_style) !== 0 ) {
                                    $parent.attr('style',container_style).css('opacity','1');
                                }
                            }
                            $this.css('opacity','1');
                            $parent.addClass('animated').addClass(animationName);
                        }
                        else{
                            ...
                            if(p_st == 'opacity:0;'){
                                if( p_st.indexOf(container_style) !== 0 ) {
                                    $this.attr('style',container_style).css('opacity','1');
                                }
                            }
                        }

                        child_style += 'opacity:0;';
                        child_style += '-webkit-animation-duration:' + animationDuration + ';';
                        child_style += '-moz-animation-duration:' + animationDuration + ';';
                        child_style += 'animation-duration:' + animationDuration + ';';
                        child_style += '-webkit-animation-iteration-count:' + animationIteration + ';';
                        ...
                        $childs.each(function( idx, elm ){
                            var $that = $(elm),
                                ...
                                new_style = '',
                                tmp_child_style = child_style;

                            if(idx == 5){
                                _idx2 = 0;
                            }
                            tmp_child_style += render_transition_delay( (animationDelay + parseFloat(_idx2 * 0.1)).toFixed(3) );
    ...
    LA.component.HotSpotImages = function(el){
        ...
        var init = function(){
            ...
            $('.la_hotspot_wrap', $(el)).click(function(e){
                e.preventDefault();
    ...
    LA.core.MegaMenu = function(){
        ...
        function fix_megamenu_position( $elem, containerClass, container_width, isVerticalMenu) {
            ...
            if (!isVerticalMenu) {
                ...
                if(containerClass == 'body.camille-body'){
                    ...
                    $('li.mm-popup-wide', _that).each(function(){
                        ...
                        if( $menu_item.hasClass('mm-popup-force-fullwidth')){
                            ...
                            if(!isVerticalMenu){
                                ...
                                _containerClass = 'body.camille-body';
                            }else{
    ...
    LA.core.WooCommerce = function(){
        ...
        $document
            .on('touchend click','.wc-view-toggle span',function(){
                ...
                if(!_this.hasClass('active')){
                    ...
                    Cookies.set('camille_wc_catalog_view_mode', _mode, { expires: 2 });
                }
            });
        ...
        if(la_theme_config.single_ajax_add_cart == 'on' || la_theme_config.single_ajax_add_cart == 'yes'){
            $(document).on('submit', '.la-p-single-wrap:not(.product-type-simple) .entry-summary form.cart', function(e){
                e.preventDefault();
    ...
    LA.component.NewsletterPopup = function(el){
        var $popup = $(el),
            ...
            waitfortrigger = $popup.attr('data-waitfortrigger');
        ...
        return {
            init : function(){
                ...
                if(waitfortrigger != '1'){
                    ...
                    try{
                        if(Cookies.get('camille_dont_display_popup') == 'yes'){
                            return;
                        }
                        ...
                        setTimeout(function(){
                            LA.utils.OpenNewsletterPopup($popup, function(){
                                if($('.cbo-dont-show-popup', $popup).length && $('.cbo-dont-show-popup', $popup).is(':checked')){
                                    try {
                                        Cookies.set('camille_dont_display_popup', 'yes', { expires: backtime, path: '/' });
                                    } catch (ex){}
                                }
                            })
                        }, p_delay)
...
(function($) {
    ...
    $(function(){
        $(document)
            .on('click', 'html.touch .product_item--thumbnail-holder .woocommerce-loop-product__link, html.touchevents .product_item--thumbnail-holder .woocommerce-loop-product__link', function(e){
                if( $(window).width() < 992) {
